...
void EnumerateClasses( Il2CppClass *klass, void *user_data ) {
    ...
    if( strcmp( klass->namespaze, context->generic_class->namespaze ) != 0 || strncmp( klass->name, context->generic_class->name, strlen( context->generic_class->name ) ) != 0 ) {
        return;
    }

    uint32_t arg_count = my_klass->GetGenericArgCount( );

    if( arg_count != context->parameters.size( ) ) {
        return;
    }
    ...
    for( uint32_t i = 0; i < arg_count; i++ ) {
        ...
        if( type_param != context->parameters [ i ] ) {
            return;
        }
    }
...
bool Initialize( std::wstring game_assembly_name ) {
    auto game_assembly = LoadLibraryW( game_assembly_name.c_str( ) );
...
Class *Class::GetGeneric( std::vector< Class * > classes ) {
    ...
    if( generic_cache.count( this ) ) {
        ...
        for( const auto &[ generic_class, parameters ] : cache ) {
...
uint32_t Class::GetGenericArgCount( ) {
...
Class *Class::GetGenericArgAt( uint32_t index ) {
    if( index >= this->generic_class->context.class_inst->type_argc ) {
        return nullptr;
    }
...
String *String::NewLen( const char *str ) {
    return ( String * ) API::il2cpp_string_new_len( str, static_cast< uint32_t >( strlen( str ) ) );
}
